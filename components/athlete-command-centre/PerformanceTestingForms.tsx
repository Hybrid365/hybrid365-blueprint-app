...
import {
  ...
  type CompromisedHyroxRunKey,
  type CompromisedHyroxStationSplits,
...
export function PerformanceTestResultForm({
  ...
  const runSplits = (resultJson.runSplits ?? {}) as Record<string, string>;
  const stationSplits = (resultJson.stationSplits ?? {}) as CompromisedHyroxStationSplits;
  const stationNotes = (resultJson.stationNotes ?? {}) as Record<string, string>;

  const updateRunSplit = (key: CompromisedHyroxRunKey, value: string) => {
    setResultJson((prev) => ({
      ...prev,
      runSplits: { ...((prev.runSplits as Record<string, string>) ?? {}), [key]: value },
    }));
  };

  const updateStationSplit = (key: keyof CompromisedHyroxStationSplits, value: string) => {
    setResultJson((prev) => ({
      ...prev,
      stationSplits: {
        ...((prev.stationSplits as CompromisedHyroxStationSplits) ?? {}),
        [key]: value,
      },
    }));
  };

  const updateStationNote = (key: string, value: string) => {
    setResultJson((prev) => ({
      ...prev,
      stationNotes: { ...((prev.stationNotes as Record<string, string>) ?? {}), [key]: value },
    }));
  };

  const renderHyroxBenchmarkForm = () => (
    <div className="space-y-5">
      <p className="rounded-xl border border-cyan-500/25 bg-cyan-400/5 p-3 text-xs text-cyan-100/90">
        {COMPROMISED_HYROX_PACING_GUIDANCE}
      </p>
      ...
      <div>
        <p className="mb-2 text-xs font-bold uppercase tracking-wide text-yellow-400/80">
          Run splits
        </p>
        <div className="space-y-2">
          {COMPROMISED_HYROX_RUN_KEYS.map((key) => {
            const ctx = COMPROMISED_HYROX_RUN_CONTEXT[key];
            return (
              <label key={key} className="block rounded-xl border border-zinc-800 p-3 text-sm">
                <span className="font-semibold text-white">{ctx.label}</span>
                <span className="mt-0.5 block text-[11px] text-zinc-500">
                  After {ctx.precededBy} · before {ctx.followedBy}
                </span>
                <input
                  type="text"
                  disabled={locked || readOnly}
                  value={String(runSplits[key] ?? "")}
                  onChange={(e) => updateRunSplit(key, e.target.value)}
                  placeholder="e.g. 3:12"
                  className="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white disabled:opacity-60"
                />
              </label>
            );
          })}
      ...
      <div>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {COMPROMISED_HYROX_STATION_FIELDS.map((field) => (
            <label key={field.key} className="block text-sm">
              <span className="mb-1 block text-zinc-300">{field.label}</span>
              <input
                type="text"
                disabled={locked || readOnly}
                value={String(stationSplits[field.key] ?? "")}
                onChange={(e) => updateStationSplit(field.key, e.target.value)}
                placeholder="e.g. 4:20"
                className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white disabled:opacity-60"
              />
            </label>
          ))}
        </div>
      ...
      <div>
        <p className="mb-2 text-xs font-bold uppercase tracking-wide text-zinc-500">
          Optional station notes
        </p>
        <div className="grid gap-2 sm:grid-cols-2">
          {[
            ["sledPauses", "Sled pauses"],
            ["farmersDrops", "Farmer’s carry drops"],
            ["lungeBreaks", "Lunge breaks"],
            ["wallBallSetBreakdown", "Wall-ball set breakdown"],
            ["wallBallNoReps", "Wall-ball no-reps"],
            ["burpeePauses", "Burpee pauses"],
            ["technicalBreakdown", "Technical breakdown"],
            ["painOrRestriction", "Pain or movement restriction"],
          ].map(([key, label]) => (
            <label key={key} className="block text-sm">
              <span className="mb-1 block text-zinc-400">{label}</span>
              <input
                type="text"
                disabled={locked || readOnly}
                value={String(stationNotes[key] ?? "")}
                onChange={(e) => updateStationNote(key, e.target.value)}
                className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white disabled:opacity-60"
              />
            </label>
          ))}
