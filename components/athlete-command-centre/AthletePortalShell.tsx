...
export function AthletePortalShell({
  ...
  /** Hide sign-out / mock toggles when embedded in coach admin preview. */
  ...
  const {
    ...
    programmeHubLive = adminPreview?.programmeHubLive ?? false,
  ...
  const isAdminPreview = adminPreviewMode || Boolean(adminPreview);
  const navVisible =
    !isAdminPreview &&
    showNav &&
    (programmeHubLive ||
  ...
  return (
    <HyroxPageShell maxWidth="max-w-7xl">
      {useMockPreview && !isAdminPreview ? <MockPreviewBanner /> : null}
      {!isAdminPreview ? (
      ...
      <div className={`min-w-0 overflow-x-hidden px-4 sm:px-6 ${navVisible ? "pb-24 pt-6 lg:pb-10" : isAdminPreview ? "py-6" : "py-6"}`}>
        {children}
      </div>
