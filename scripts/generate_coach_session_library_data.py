...
SESSIONS: list[dict] = [
    ...
    S(
        ...
        "low",
        "none",
        ["Running shoes", "Treadmill optional", "HR monitor optional"],
        ["aerobic_base", "recovery_support", "easy_day", "between_hard_sessions", "support_days"],
        ["turned_into_tempo", "legs_heavy_unmanaged"],
        ["run", "easy", "z2", "zone_2", "aerobic", "aerobic_base", "aerobic_run", "recovery_support", "recovery_run", "low_stress", "endurance", "support_session"],
        ["durationMinutes", "targetRPE", "targetPace", "coachNote"],
        rx(
            "Build aerobic base, improve recovery capacity and add running volume without compromising key sessions.",
            ["5–10 min very easy jog/walk build"],
            [
                "Default: 30–60 min easy Zone 2 run",
                "Duration options: 30 · 40 · 45 · 60 min (75 min advanced only if recovery allows)",
                "Treadmill: 1% incline unless calf tightness/niggles suggest otherwise",
                "Beginner: 20–35 min easy jog/walk as needed",
                "Intermediate: 35–55 min easy Z2 run",
                "Advanced: 50–75 min easy Z2 run — only if it does not affect key sessions",
            ],
            ["5 min easy walk/jog if needed"],
            ["Total time", "Distance", "Average pace", "Average HR if available", "RPE", "Notes on how controlled it felt"],
            "This is not a hard session. The goal is aerobic volume, recovery support and durability. Finish feeling like you could continue. Use on easy/support days — good after hard threshold or lower-body days. Do not let athletes turn this into tempo/grey-zone work.",
            targetPace="Easy conversational pace — full sentences. Do not chase pace. If tired, keep it easier.",
            targetHR="Zone 2 / conversational. Roughly 60–75% max HR if using HR — avoid drifting into threshold.",
            targetRPE="3–5/10",
            safetyNote="If legs are heavy or HR is unusually high, slow down or reduce duration.",
        ),
        qualityRunMinutes=45,
        runDistanceKm=8,
        similarSessions=["easy-shakeout", "easy-bike"],
        progressionOptions=[
            "Beginner: 20–35 min easy jog/walk as needed",
            "Intermediate: 35–55 min easy Z2 run",
            "Advanced: 50–75 min easy Z2 run if recovery allows",
        ],
        regressionOptions=["Reduce duration to 20–30 min", "Add walk breaks — keep conversational pace"],
        duration="30–75 min",
        source="Hybrid365 library",
    ),
    ...
    # 28–32 upper_grip
    S("pull-push-emom", "hyrox_strength_upper_emom", "Upper Body Density EMOM", "upper_grip", "upper_strength_support", "all", 45, False, "upper_strength_support", "strength", "moderate", "low", ["Pull-up bar", "Dumbbells", "Bands"], ["gym_based_aerobic", "upper_support", "grip"], ["shoulder_pain", "elbow_pain", "heavy_wb_after"], ["upper-strength", "grip", "emom"], ["reps", "load"], rx("Upper density EMOM + optional grip holds.", ["5–8 min easy erg", "Band pull-aparts", "Scap pull-ups", "Push-up walkouts", "Light DB holds"], ["10 min EMOM pull/push", "Optional DB max holds", "Scaling by level"], ["Shoulder mobility", "Lat stretch", "Grip release"], ["Variation", "Reps per round", "DB hold load/time", "RPE", "Grip fatigue"], "Controlled density — not failure if hard Hyrox work in 24–48h.", targetRPE="6–7", safetyNote="Reduce reps or use assisted options if quality drops."), strengthMinutes=15, stationVolume=10, isOptionalAddOn=True),
