...
import {
  HYBRID_PERFORMANCE_PLATFORM,
  type HybridPerformancePlatformSlide,
...
function PlatformSlide({
  ...
  slide: HybridPerformancePlatformSlide;
  priority?: boolean;
}) {
...
export function HybridPerformancePlatform() {
  const scrollerRef = useRef<HTMLDivElement>(null);
  const [index, setIndex] = useState(0);
  const slides = HYBRID_PERFORMANCE_PLATFORM.slides;
  ...
  return (
    <HomepageSection id="platform" variant="default" className="border-b-0 !py-12 sm:!py-16">
      <div className="mx-auto max-w-3xl text-center lg:mx-0 lg:text-left">
        <HomepageEyebrow>{HYBRID_PERFORMANCE_PLATFORM.eyebrow}</HomepageEyebrow>
        <HomepageHeading className="text-[clamp(1.7rem,5.5vw,2.75rem)]">
          {HYBRID_PERFORMANCE_PLATFORM.headline}
        </HomepageHeading>
      </div>
